...
Deployment uses **GitHub + Render** for both tasks. The root [`render.yaml`](./render.yaml) defines four services: a MongoDB-backed Task 01 API and frontend, plus the in-memory Task 02 API and frontend. The root [GitHub Actions workflow](./.github/workflows/deploy.yml) checks both applications on pull requests and pushes, and can trigger Render deploy hooks after successful pushes to `main`.
